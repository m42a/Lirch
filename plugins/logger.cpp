...
void run(plugin_pipe pipe, std::string name)
{
	...
	pipe.write<registration_message>(LIRCH_MSG_PRI_REG_MAX, name, LIRCH_MSG_TYPE_DISPLAY);
	pipe.write<registration_message>(LIRCH_MSG_PRI_REG_MAX, name, LIRCH_MSG_TYPE_LOGGING);
	...
	while (true) {
		...
		else if (registration_status * internals = msg.try_extract<registration_status>())
		{
			...
			if (!internals->status)
			{
				...
				pipe.write<registration_message>(internals->priority-1, name, internals->type);
			}
		}

		//logs the display messages, provided logging is enabled
		else if (display_message * internals = msg.try_extract<display_message>())
		{
		...
		//change settings
		else if (logging_message * internals = msg.try_extract<logging_message>()) {
